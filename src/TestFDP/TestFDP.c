...
bool testVirtualSyscallBP(FDP_SHM* pFDP, FDP_BreakpointType BreakpointType)
{
    ...
    int breakpointId = FDP_SetBreakpoint(pFDP, 0, BreakpointType, -1, FDP_EXECUTE_BP, FDP_VIRTUAL_ADDRESS, originalMSRValue, 1);
    if (breakpointId < 0){
    ...
    while (TimerOut == false){
        if (FDP_GetStateChanged(pFDP)){
            ...
            if (state & FDP_STATE_PAUSED
                ...
                breakpointId = FDP_SetBreakpoint(pFDP, 0, BreakpointType, -1, FDP_EXECUTE_BP, FDP_VIRTUAL_ADDRESS, originalMSRValue, 1);
                if (breakpointId < 0){
                    printf("Failed to insert page breakpoint !\n");
                    return false;
                }
...
bool testPhysicalSyscallBP(FDP_SHM* pFDP, FDP_BreakpointType BreakpointType){
    ...
    int breakpointId = FDP_SetBreakpoint(pFDP, 0, BreakpointType, -1, FDP_EXECUTE_BP, FDP_PHYSICAL_ADDRESS, physicalLSTAR, 1);
    if (breakpointId < 0){
        printf("Failed to insert page breakpoint !\n");
        return false;
    }
    ...
    while (TimerOut == false){
        if (FDP_GetStateChanged(pFDP)){
            ...
            if (state & FDP_STATE_BREAKPOINT_HIT
                ...
                breakpointId = FDP_SetBreakpoint(pFDP, 0, BreakpointType, -1, FDP_EXECUTE_BP, FDP_PHYSICAL_ADDRESS, physicalLSTAR, 1);
                if (breakpointId < 0){
                    printf("Failed to insert page breakpoint !\n");
                    return false;
                }
...
bool testMultiplePhysicalSyscallBP(FDP_SHM* pFDP, FDP_BreakpointType BreakpointType){
    ...
    int breakpointId[10];
    for (int j = 0; j < 10; j++){
        breakpointId[j] = FDP_SetBreakpoint(pFDP, 0, BreakpointType, -1, FDP_EXECUTE_BP, FDP_PHYSICAL_ADDRESS, physicalLSTAR + j, 1);
        if (breakpointId[j] < 0){
            printf("Failed to insert page breakpoint !\n");
            return false;
        }
...
bool testMultipleVirtualSyscallBP(FDP_SHM* pFDP, FDP_BreakpointType BreakpointType){
    ...
    int breakpointId[10];
    for (int j = 0; j < 10; j++){
        breakpointId[j] = FDP_SetBreakpoint(pFDP, 0, BreakpointType, -1, FDP_EXECUTE_BP, FDP_VIRTUAL_ADDRESS, originalMSRValue + j, 1);
        if (breakpointId[j] < 0){
            printf("Failed to insert page breakpoint !\n");
            return false;
        }
...
bool testLargeVirtualPageSyscallBP(FDP_SHM* pFDP){
    ...
    int breakpointId = FDP_SetBreakpoint(pFDP, 0, FDP_PAGEHBP, -1, FDP_EXECUTE_BP, FDP_VIRTUAL_ADDRESS, originalMSRValue, 4096*30);
    if (breakpointId < 0){
        printf("Failed to insert page breakpoint !\n");
        return false;
    }
    ...
    while (i < 10){
        ...
        if (state & FDP_STATE_BREAKPOINT_HIT
            ...
            breakpointId = FDP_SetBreakpoint(pFDP, 0, FDP_PAGEHBP, -1, FDP_EXECUTE_BP, FDP_VIRTUAL_ADDRESS, originalMSRValue, 4096 * 30);
            if (breakpointId < 0){
                printf("Failed to insert page breakpoint !\n");
                return false;
            }
...
bool testLargePhysicalPageSyscallBP(FDP_SHM* pFDP)
{
    ...
    int breakpointId = FDP_SetBreakpoint(pFDP, 0, FDP_PAGEHBP, -1, FDP_EXECUTE_BP, FDP_PHYSICAL_ADDRESS, physicalLSTAR, 4096 * 30);
    if (breakpointId < 0){
        printf("Failed to insert page breakpoint !\n");
        return false;
    }
...
bool testState(FDP_SHM* pFDP)
{
    ...
    int breakpointId = FDP_SetBreakpoint(pFDP, 0, FDP_SOFTHBP, -1, FDP_EXECUTE_BP, FDP_VIRTUAL_ADDRESS, originalMSRValue, 1);
    if (breakpointId < 0){
        printf("Failed to insert page breakpoint !\n");
        return false;
    }
